...
class cfgAmmo
{
	...
	//Rocket ammo class name example
	class 22nd_R_HEAT_SACLOS: OPTRE_M41_Rocket_HEAT_G  
	{
			
			model = "\OPTRE_Weapons\Rockets\M41_rocket.p3d"; 
			warheadName ="TandemHEAT";
			ace_vehicle_damage_incendiary = 1; //Chance of it setting vehicles on fire if it pens
			submunitionAmmo = "ammo_Penetrator_Vorona"; //Ammo that activates when it hits target
			effectsMissile = "missile2"; //smoke effect
			effectsMissileInit = "RocketBackEffectsRPG"; //backblast effect
			aiAmmoUsageFlags = "128 + 256 + 512"; //Vehicles, Aircraft, Armor
			lockType = 1; //Beam-riding
			airLock = 0; // can lock air vehicles?
			irLock = 1; //Can lock IR?
			laserLock = 0; //Can lock laser?
			ace_frag_skip = 0; // see ace fragmentation framework document
			...
			hit = 50; //damage value
			indirectHit = 25; //indirect (splash) damage value
			indirectHitRange = 6; //Indirect damage range

			cmImmunity = 1; // Specifies a missile's resistance to countermeasures (such as chaff and flares). 1 = is resistant everytime a chaff or flare is fired. 0.5 = half the time.
			weaponLockSystem = "1 + 2 + 16"; //https://community.bistudio.com/wiki/Arma_3:_Targeting_Config_Reference for the below
			manualControl = 1;
